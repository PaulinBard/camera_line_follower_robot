...
def transfo(image,target_color):
    ...
    if target_color==color[1]:
        ...
        y= cv2.countNonZero(red_mask)
        return mask,x,y
    if target_color==color[2]:
        hsv_image = cv2.cvtColor(image, cv2.COLOR_BGR2HSV)

        lower_red = np.array([23, 100, 100])  # Limite inférieure du rouge dans l'espace HSV
        upper_red = np.array([60, 230, 230])  # Limite supérieure du rouge dans l'espace HSV
...
def deplacementV2(robot,x,frame):
    ...
    bonus =  erreur*10/(frame.shape[1]/2)
    if x ==-1:
        ctrl.stop(robot)
    else:
        print("bonus :",bonus)
        go_side_forward(robot,.85*vitesse,bonus)
