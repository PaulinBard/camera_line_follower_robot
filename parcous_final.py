...
def mainV1 ():
    ...
    while True:
        with DxlIO(port) as dxl_io:
            ...
            if erreur < -borne_erreur:
                print("erreur < -40")
                ctrl.rotation(dxl_io, -rot_erreur)
            elif erreur > borne_erreur:
                print("erreur 40")
                ctrl.rotation(dxl_io, rot_erreur)
            else :
                print("forward")
                ctrl.go_forward(dxl_io,.85*vitesse)    
                time.sleep(.75)
            # Display the resulting frame
    #        cv2.imshow('frame', gray)
...
def mainV2 ():
    ...
    while True:
        with DxlIO(port) as dxl_io:
            ...
            # # Calculer la largeur de la partie gauche à conserver (2/3 de la largeur)
            # largeur_conservee = int(largeur * 2 / 3)

            # # Couper l'image pour conserver la partie droite (1/3 de l'image)
            # image_coupee = frame[:, :largeur_conservee]
            hauteur, largeur, canaux = frame.shape

            debut_x = 0 # 25% de la largeur à partir de la gauche
            fin_x =largeur# 75% de la largeur à partir de la gauche#############c mon haut
