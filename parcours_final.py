...
def deplacementV2(robot,x,frame):
    ...
    bonus =  erreur*100/(frame.shape[1]/2)
    if x ==-1:
        ctrl.stop(robot)
    else:
        print("bonus :",bonus)
        ctrl.go_side_forward(robot,.85*vitesse,bonus)
    ...
    while True:
        with DxlIO(port) as dxl_io:
            ...
            if x <= 20:
                ...
                cpt_tour+=1
            if cpt_tour == 1 :
                gray ,y,x= transfo(frame,"noir")#cv.cvtColor(frame, cv.COLOR_BGR2GRAY)
                deplacementV1(dxl_io,x,frame)

            if cpt_tour == 2 : 
                gray ,y,x= transfo(frame,"vert")#cv.cvtColor(frame, cv.COLOR_BGR2GRAY)
                deplacementV1(dxl_io,x,frame)
